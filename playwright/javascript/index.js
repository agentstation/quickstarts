...
async function main(existingWorkstation = null, existingPage = null) {
  ...
  try {
    if (!existingPage) {
      ...
      console.log('📱 Getting existing browser page...');
      const context = workstationManager.browser.contexts()[0];
      const pages = await context.pages();
      page = pages[0]; // Use the existing page
  ...
  try {
    while (true) {
      ...
      const answer = await new Promise(resolve => {
        rl.question('Please enter (y/n): ', resolve);
      });
